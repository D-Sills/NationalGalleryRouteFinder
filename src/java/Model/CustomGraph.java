...
public class CustomGraph {
		public static List<CostedPath> allPathsFinal;
		...
		public static <T> CostedPath searchGraphDepthFirstCheapestPath(GraphNode<?> from, List<GraphNode<Room>> encountered,
		                                                               int totalCost, T lookingfor){
				if(from.getData().equals(lookingfor)){ //Found it - end of path
				...
				encountered.add((GraphNode<Room>) from);
				List<CostedPath> allPaths=new ArrayList<>(); //Collection for all candidate costed paths from this node
				for(GraphLink adjLink : from.getAdjList()) //For every adjacent node
...
//Create a new CostedPath from this node to the searched for item (if a valid path exists)
								CostedPath temp=searchGraphDepthFirstCheapestPath(adjLink.getDestNode(),new ArrayList<>(encountered),
												totalCost+adjLink.getCost(),lookingfor);
				...
				CustomGraph.allPathsFinal = allPaths;
...
//We're not at the goal node yet, so...
		...
		public static <T> List<GraphNode<?>> findPathBreadthFirst(List<List<GraphNode<?>>> agenda,
				...
				for(GraphLink adjNode : currentNode.getAdjList()) //For each adjacent node
						if(!encountered.contains(adjNode.getDestNode())) { //If it hasn't already been encountered
								List<GraphNode<?>> newPath=new ArrayList<>(nextPath); //Create a new path list as a copy of
//the current/next path
								newPath.add(0,adjNode.getDestNode()); //And add the adjacent node to the front of the new copy
